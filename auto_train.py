...
if __name__ == '__main__':
    ...
    batch = tf.train.shuffle_batch([in_pointa, label], batch_size=args.batch_size, capacity=800, num_threads=2, min_after_dequeue=200)
    ...
    with tf.Session() as sess:
        ...
        for i in range(args.epochs):
            p, l = sess.run(batch)
            loss_val, _ = sess.run([cost, opt], feed_dict={points: p, labels:l})
            
            # Print training accuracy every 100 epochs
            if (i+1) % 100 == 0:
                print('loss val {}: {:.2f}'.format(i+1, sess.run(loss_val, feed_dict={points: p, labels:l})))
